...
> **IMPORTANT**: Whenever you make any change to the codebase — adding features, removing files, fixing bugs, updating config — you **MUST** also update **CURRENT.md** and **CHANGELOG.md** to reflect those changes. These files are the source of truth for what the project looks like right now.
...
4. Create ABI file in `frontend/abi/`
5. Add to `frontend/abi/index.ts` barrel export
6. Create hooks in `frontend/hooks/`
...
4. Create any needed components in `frontend/components/`
5. **Update CURRENT.md and CHANGELOG.md**

### Adding a New Hook

1. Create in `frontend/hooks/use<Feature>.ts`
2. Use Wagmi's `useReadContract` / `useWriteContract`
3. Export from `frontend/hooks/index.ts`
4. **Update CURRENT.md and CHANGELOG.md**
...
1. **SSR Hydration**: Use `mounted` state pattern for wallet-dependent UI to avoid hydration mismatches.
