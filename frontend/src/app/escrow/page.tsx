  ...
  return (
    <div>
      ...
      <div className="flex gap-1 mb-6">
        <TabButton active={view === "shipper"} onClick={() => setView("shipper")}>
          As Shipper ({shipperEscrows?.length || 0})
        </TabButton>
        <TabButton active={view === "carrier"} onClick={() => setView("carrier")}>
...
function EscrowList({ ids, role }: { ids: readonly bigint[]; role: "shipper" | "carrier" }) {
  if (!ids || ids.length === 0) {
    return (
      <div className="card">
...
function CreateEscrowForm({ onSuccess }: { onSuccess: () => void }) {
  ...
  const { create, isPending: isCreating, isConfirming, isSuccess } = useCreateEscrow();
  ...
  return (
    <div className="card max-w-xl">
      <p className="text-xs text-neutral-500 uppercase tracking-wide mb-4">New Freight Escrow</p>
        ...
        <div className="flex gap-2 pt-2">
          {!approved ? (
            ...
            <button
              ...
              disabled={!form.carrier || !form.amount || isCreating || isConfirming}
